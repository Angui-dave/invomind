...
export default function ReportsPage() {
  const currency = ORG_SETTINGS.defaultCurrency;
  const invoices = getInvoices();
  const revenueSeries = revenueByMonth(12);

  const collected = useMemo(() => totalCollected(), []);
  const billedTtc = useMemo(() => billedRevenueTtc(), []);
  const salesHt = useMemo(() => billedRevenueHt(), []);
  const expensesHt = useMemo(() => expensesTotalHt(EXPENSES), []);
  const expensesTtc = useMemo(
  ...
  const expensesByCategory = useMemo(() => {
    return EXPENSE_CATEGORIES.map((cat) => ({
      name: cat.name,
      amount: EXPENSES.filter((e) => e.categoryId === cat.id).reduce(
        (s, e) => s + e.amount,
        0,
      ),
      fill: cat.color,
  ...
  const vatCollectedAmount = useMemo(() => vatCollected(), []);
  const vatDeductible = useMemo(
    () =>
      EXPENSES.filter((e) => e.taxDeductible).reduce(
        (s, e) => s + e.taxAmount,
        0,
      ),
    [],
  ...
  const vatRows = useMemo(() => vatByRate(), []);

  const paymentsByCurrency = useMemo(
    () => sumByCurrency(EXPENSES.map((e) => ({ amount: e.amount, currency: e.currency }))),
    [],
  );
